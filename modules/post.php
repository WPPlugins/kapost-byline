...
function kapost_byline_update_post_data($data, $custom_fields, $blog_id=0)
{
	...
	if(in_array($data['post_status'], array('draft', 'auto-draft')))
	{
		...
		else if($settings['sql_server_compat'] != 'on')
		{
			// SQL Server doesn't like *null* dates so just leave the date as-is
